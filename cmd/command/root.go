...
Behavior:
  ...
  - If --host is not provided, containerctl connects to the local Docker API-compatible socket in the following order of precedence:
	1. If DOCKER_HOST, DOCKER_API_VERSION, DOCKER_CERT_PATH or DOCKER_TLS_VERIFY environment variable are set, they are also used.
	2. If --docker-host is provided, it is used as the host.
  ...
  - If --host is provided, the connection is done securely over SSH.
    ...
    - To use a Docker engine API-compatible service, provide a --docker-host URL (e.g. unix:///run/user/<UID>/podman/podman.sock ).   
	  If --docker-host is not provided the default (unix:///var/run/docker.sock) is used.
	- The Docker Engine API socket is determined in the following order of precedence 
	  (DOCKER_HOST environment variable is ignored when --host is provided):
		1. If --docker-host is provided, it is used as the docker unix socket.
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&dockerhost, "docker-host", "/var/run/docker.sock", "The Docker API socket to connect to. Ignored if --host is provided.")
